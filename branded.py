...
for idx in restricciones.index:
    ...
    if match:
        ...
        if p < q:
            ...
            C.loc[idx, p+1:max_c+1] = 1  # np.arange funciona como índice para asignación
            C.loc[idx, l2+max_c+1:l2+q+1] = -1
        else:
            # Si p > q, poner -1s desde p+1 hasta max_c y 1s desde max_c+1 hasta q
            C.loc[idx, q+1:max_c+1] = -1
            C.loc[idx, l2+max_c+1:l2+p+1] = 1

# Mostrar el resultado final de C
print("Matriz C")
print(C)

#Sistema de ecuaciones para C

# Definir la función objetivo (en este caso, no importa, solo queremos que la suma sea positiva)
# Usaremos un array de ceros, ya que no estamos maximizando ni minimizando nada específico.
objetivo = np.zeros(C.shape[1])

# Crear la matriz de desigualdad (Ax <= b)
# Queremos que cada fila de C tenga una suma mayor que 0:
A_ineq = -C  # Negamos C para convertir la inecuación en Ax <= b
b_ineq = -np.ones(C.shape[0])  # Cada fila debe ser mayor que 0

# Resolvemos el problema usando linprog
result = linprog(objetivo, A_ub=A_ineq, b_ub=b_ineq, method='highs')

# Verificamos el resultado
if result.success:
    print("Solución encontrada:")
    print(result.x)  # Valores óptimos que cumplen las inecuaciones
else:
